...
export const removeFromFav = (product) => {
  // console.log("product remov", product);
...
export const isAdmin = (id) => {
  const admin = ["XxiMi7QZaJTdjM9pLrgCYwJaX7G3"]; 
  // const admin = [process.env.UID_USER]; // TODO 
  let exist = false;
  if (id) {
    admin.map((a) => {
      if (a === id) {
...
export const setCountFav = (countFav) => {
  return {
    type: actionTypes.SET_COUNT_FAV,
    payload: {
      countFav: countFav
    }
  }
